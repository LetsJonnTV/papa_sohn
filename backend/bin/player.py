...
class Player:
    def __init__(self, username, passwort, res_stats, char_stats):
        self.__username__ = username
        self.__passwort__ = passwort
        self.__res_stats__ = res_stats
        self.__Charakter_stats__ = char_stats

    def returnen(self):
        dic = {"Username": self.__username__, "Passwort": self.__passwort__, "Res_Stats": self.__res_stats__, "Char_Stats": self.__Charakter_stats__}


main_char_stats = Charakter_Stats(geld, xp, level)
main_res_stats = Resurcen_Stats(user_res[1][2], user_res[1][3])
main_player = Player(user[0],user[1], main_res_stats.returnen(), main_char_stats.returnen())